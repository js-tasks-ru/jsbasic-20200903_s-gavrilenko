...
1. Создает новое модальное окно на основе компонента `Modal`. x
2. Добавляет заголовок `"Your order"` в созданное модальное окно. x
...
```
4. Вставляет эту верстку в тело модального окна. x
...
x
6. Добавляет необходимый обработчик, чтобы при событии `submit` на элементе формы с классом `cart-form`, вызывался метод `onSubmit` данного компонента. При этом при вызове этого метода, ему нужно будет не забыть передать в качестве аргумента объект события: x
x
```js
